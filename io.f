C***********************************************************************
     ...
     &                 ISYM,MAXTRM)
      ...
      REAL*8 rand,mass,mass1
      real*8 vlocal,limiter
      real*8 iphix,iphiy
...
c..axisymmetric start from formatted files (ie generated from the 2D hydrocode)
     ...
     &     rho_axi(jmax2,kmax2),eps_axi(jmax2,kmax2)
      real*8 starphi_axi(jmax2,kmax2)
...
C              PERTURB IT WITH RANDOM PERTURBATION.
   ...
   99 FORMAT(1PE15.8)
  100 FORMAT(1P2E15.8)
  101 FORMAT(' ',12X,'CLOUD RADIUS =',1PE11.4,' CM',/,12X,'CLOUD DENSITY
     & =',1PE11.4,' GM CM-3',/,5X,'BOUNDARY TEMPERATURE =',1PE11.4,
     &' DEGREES K',//,10X,'CONST. PRESSURE =',1PE11.4,' DYNE/CM2',/,16X,
     &'TIME STEP =',1PE11.4,' YEARS',/,13X,'STARTING AGE =',1PE11.4,
     &' YEARS',//,9X,'INITIAL ROTATION =',1PE11.4,' SEC-1',///)
  130 FORMAT(27X,'MODE',6X,'AMP',8X,'WR',8X,'WZ',6X,'XPEAK',6X,
     &'FMASS',/,10X,'INITIAL KICK',5X,F4.1,4(5X,F5.3),4X,1PE8.1,///)
  102 FORMAT(2(I8,1X),7(I6,1X))
 ...
 1044 FORMAT('1THIS WILL BE AN ENERGY EQUATION EVOLUTION, FROM TIMESTEP
  ...
  120 FORMAT('SEQUENCE USES A CONSTANT PRESSURE BOUNDARY CONDITION.',//)
  121 FORMAT('SEQUENCE USES A CONSTANT VOLUME BOUNDARY CONDITION.',//)
...
10040 FORMAT('  NPRIME ',1PE20.12)
 ...
 16   FORMAT(3X,1PE22.15,2X,8(1PE22.15,2X),2I4)
 17   FORMAT(8(1pe22.15,2x))

 106  FORMAT(//,10X,'ROTATING POLYTROPE...  N =',0PF5.2,/,27X,
     &     '1 + 1/N =',1PE10.3,/,33X,'K =',1PE10.3/)
...
c      IF(PINDEX.NE.XN) WRITE(3,137)
  137 FORMAT(//,'PINDEX.NE.XN... CHECK FILE',///)
c      IF(PINDEX.NE.XN)STOP
...
cbkp...added tempc here
     ...
     &        JREQ,OMMAX
            if (jmin.gt.2) read(7,iostat=ios) tmassini,tmass,tmassadd,
     &         tmassout,tmassacc,totcool,totdflux,totheat,totirr,etotfl,
     &         eflufftot  !ACB

            if (ios /= 0) then
               print*,'Did not find etotfl or eflufftot'
               backspace(7)
               read(7,iostat=ios) tmassini,tmass,tmassadd,
     &         tmassout,tmassacc,totcool,totdflux,totheat,totirr
               if (ios /= 0) then
...
c      IF(CONSTP.GT.0.0) WRITE(3,120)
c      IF(CONSTP.LE.0.0) WRITE(3,121)
...
c
      ...
      if(irtype.eq.1) write(3,24)
      if(irtype.eq.0) write(3,241)
 ...
 232  format(' Flux Limited Diffusion and Shining Atmosphere')  
 24   format(' Irradiation is ON')
 241  format(' Irradiation is OFF')
      write(3,*)
...
c...  only and absorption plus scattering.
      ...
      open(unit=70,file='mmw_new.dat',status='old',form
     &     ='formatted')
      do i=1,itable
         do n=1,itable
      ...
      open(unit=71,file='rosseland.abpoll.p3p5.amax1.dat',status='old'
     &     ,form='formatted')
      do i=1,itable
         do n=1,itable
      ...
      open(unit=72,file='planck.abpoll.p3p5.amax1.dat', status='old',
     &     form='formatted')
      do i=1,itable
         do n=1,itable
      ...
      open(unit=73,file='planck.abpoll.p3p5.amax1.ts4000.dat', status
     &     ='old',form='formatted')
      do i=1,itable
         do n=1,itable
      ...
      open(unit=74,file='plancktot.abpoll.p3p5.amax1.ts4000.dat',
...
!$OMP&  SHARED(delt,tmassacc,zof3n,dtheta,tmass,mass_star)
      if (jmin.gt.2) then
C        recalculate the total mass, acm2000
!$OMP MASTER
          tmass=0.d0
!$OMP END MASTER
...
!!!!!!!!!!!!!!!! For the indirect potential !!!!!!!!!!!!!!!!!
     ...
     &                    *iphiy
                     phi(j,k,l)= phi(j,k,l)+ipot(j,k,l)+starphi(j,k,l)+ ! ELIMINATED diskphi. ACB.
     &                    tinphi(j,k,l)
                  enddo
               enddo
            enddo
...
!$OMP BARRIER                  
     ...
     &                  rof3n,zof3n,dtheta,JMAX,KMAX,LMAX,JMIN,.true.,
     &                   .false.)
...
!$OMP END DO
         ...
         CASE DEFAULT
            ...
            print*,'Hey partner, what am I supposed to do with 
     &              this star?'
            STOP
...
c
      IF((ITYPE.EQ.7).OR.(ITYPE.EQ.97).OR.(ITYPE.EQ.98)) THEN
...
c     end of bogus initializations.
 ...
 106  FORMAT(4X,1PE12.4,' YEARS',6X,1P5E12.4,I4,2I3,2X,1P2E12.4,/)
 107  FORMAT(20X,'PDV =',1PE12.4,5X,'R-CLOUD =',1PE12.4)
 118  FORMAT(///)
 119  FORMAT(2I4,1P5E11.3,2X,1P5E11.3)
 120  FORMAT(2I4,1P5E11.3,2X,1P3E11.3)
...
!$OMP END PARALLEL
      ...
      CD=rho(2,2,1)
      PDV=0.d0
   85 ETOT=EGRAV+EKIN+ENEW+ELOST-PDV
      ECHECK=0.d0
      if (JSP.gt.0) then
     ...
     &PHICHK,KLOCAT
      write(3,106)TYEAR,TOTJN,EROT,RZKIN,CD,DMAX,JD,KD,LD,ECHECK
...
crpl..xxxtodo: fix filenames so they are not hard coded.

      IF((MOD(ITSTEP,(ISTOR2)).eq.0).and.(ITSTEP.ne.ITSTOP))THEN
