...
!     Get planet positions
      ...
      planet_x = (r(jreq)*initial_r*cos(initial_phi))/Rdiskau
      planet_y = (r(jreq)*initial_r*sin(initial_phi))/Rdiskau
      CALL planet_force(planet_x,planet_y,planet_ax,planet_ay)
      diskrforce = sqrt(planet_ax**2+planet_ay**2)

!     Get circular velocities

      v_c = sqrt((diskrforce*initial_r*r(jreq)/Rdiskau))
      planet_vx = -v_c*sin(initial_phi)
      planet_vy = v_c*cos(initial_phi)
...
!     First get planet position on the grid
      ...
      IF((planet_j.eq.JMAX+1).or.(planet_j.eq.JMIN)) STOP
